...
import static Yelp.DependancyParser.Featureselection.AllinOne;
import static Yelp.DependancyParser.Featureselection.GetSelectedTFIDFFeatures;
import static Yelp.DependancyParser.Featureselection.GneerateTrainMatrix;
import static Yelp.DependancyParser.Featureselection.LoadComments;
...
import static Yelp.DependancyParser.TrainTFDP.Exeprocess;
import static Yelp.DependancyParser.dependacytrainmatrix.GenerateHybridFset;
import static Yelp.DependancyParser.dependacytrainmatrix.Test;
...
public class DeprendancyMasterPage extends javax.swing.JFrame {
    ...
    private void jButton5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton5ActionPerformed
           ...
           for (int startpoint = 0; startpoint < foodF.size(); startpoint++) {
            ...
            for (int startpoint = 0; startpoint < worthinessF.size(); startpoint++) {
                ...
                for (int first = 0; first < LemmasTokens.length; first++) {
                    String currentFeature = LemmasTokens[first];
                    AllinOne.add(currentFeature);
                    int Count = GetDependancyFrequency.worthFrequency(currentFeature);
                    //  if(Count >1){
                    System.out.println("id \t" + cID + "\t Term \t " + currentFeature + "\t TF \t" + Count);
                    Statement st = con.createStatement();
                    st.executeUpdate("insert into dpfrequency (id, aspect, term, frequency) values ('" + cID + "','Worthiness','" + currentFeature + "'," + Count + ")");
                    //  }
                }
            }
            ...
            for (int startpoint = 0; startpoint < restaurantF.size(); startpoint++) {
                String[] miscellaneousparts = restaurantF.get(startpoint).toString().split("#");
                String cID = miscellaneousparts[0];
                String[] LemmasTokens = miscellaneousparts[1].split(",");
                for (int first = 0; first < LemmasTokens.length; first++) {
                    String currentFeature = LemmasTokens[first];
                    AllinOne.add(currentFeature);
                    int Count = GetDependancyFrequency.restaurantFrequency(currentFeature);
                    //  if(Count >1){
                    System.out.println("id \t" + cID + "\t Term \t " + currentFeature + "\t TF \t" + Count);
                    Statement st = con.createStatement();
                    st.executeUpdate("insert into dpfrequency (id, aspect, term, frequency) values ('" + cID + "','restaurant','" + currentFeature + "'," + Count + ")");
                    //  }
                }
            }
        ...
        GetSelectedTFIDFFeatures(); // select features from dpFrequency  table
        Exeprocess(); // insert into dpcooccurance
    ...
    private void jButton7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton7ActionPerformed
        // TODO add your handling code here:
        // TODO add your handling code here:  
      //  GneerateTrainMatrix(); // generate dptrainmatrix
       // GenerateHybridFset(); // insert data into hybridmatrix and collects from dptrainmatrix + trainmatrix
